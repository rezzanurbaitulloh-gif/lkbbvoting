      ...
      <main className="flex-1 pb-[72px] md:pb-0">
        ...
        <div className="mx-auto max-w-[1280px] px-3 sm:px-4 md:px-6 py-8 space-y-8">
          {tiers.map(tier=> {
            ...
            return (
              <div key={tier} className="rounded-[16px] border border-white/[0.06] bg-white/[0.03] backdrop-blur p-6">
                <h2 className="text-xs font-black tracking-[0.14em] text-gold">{tier.toUpperCase()}</h2>
                <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {list.map(s=> (
                    <div key={s.id} className="rounded-xl border border-white/[0.06] bg-white/[0.04] backdrop-blur p-4 grid place-items-center h-[84px] text-center">
          ...
          <div className="rounded-[16px] border border-dashed border-white/[0.06] bg-white/[0.04] backdrop-blur/20 p-6 text-center">
            ...
            <a href="/kontak" className="inline-flex mt-3 rounded-full bg-white/[0.07] backdrop-blur border border-white/10 text-white px-5 py-2 text-xs font-bold">Hubungi Kami</a>
          </div>
        </div>
      </main>
      <Footer />

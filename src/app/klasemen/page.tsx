...
export default function KlasemenPage(){
  ...
  const ranked = getRankedPeletons(tab)
      ...
      <main className="flex-1 pb-[72px] md:pb-0">
        <div className="border-b border-border bg-[#08090B] text-white relative overflow-hidden">
          ...
          <div className="relative mx-auto max-w-[1280px] px-4 md:px-6 py-8">
            ...
            <p className="mt-2 max-w-xl text-sm text-white/60">Peringkat berdasarkan dukungan terverifikasi. Hanya urutan yang ditampilkan ke publik — jumlah dukungan tidak dipublikasikan.</p>
        ...
        <div className="mx-auto max-w-[1280px] px-4 md:px-6 py-6">
              ...
              return (
                <div key={p.id} className={`relative overflow-hidden rounded-[20px] border-2 p-4 flex flex-col ${podiumStyle}`}>
                  ...
                  <div className="mt-3 flex gap-3">
                    <img src={p.image} alt="" className="h-16 w-16 rounded-xl object-cover border border-border" />
                  ...
                  {i===0 && <div className="absolute -right-6 -top-6 h-20 w-20 rounded-full bg-[#C9A86A]/10 blur-2xl" />}
                </div>
              )
            })}
          </div>
                  ...
                  <div key={p.id} className="grid md:grid-cols-[64px_1fr_180px_120px] gap-3 px-4 py-3 items-center hover:bg-muted/40 transition-colors">
                    ...
                    <Link href={`/peleton/${p.slug}`} className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground truncate md:hidden">{p.city} • {p.category}</div>
                        <div className="hidden md:block text-xs text-muted-foreground truncate">{p.school}</div>
                      </div>
                    </Link>
                    <div className="hidden md:block text-sm text-muted-foreground">{p.city} • {p.category}</div>
          ...
          <div className="mt-6 rounded-xl border border-[#C9A86A20] bg-[#C9A86A0A] p-4 text-center">
            <p className="text-xs leading-relaxed text-muted-foreground">Klasemen diperbarui otomatis setelah setiap transaksi dukungan yang terverifikasi. Jumlah dukungan tidak ditampilkan ke publik sesuai prinsip kompetisi.</p>
          </div>
        </div>
      </main>
      <Footer />

  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 pb-[72px] md:pb-0">
        <div className="border-b border-white/[0.06] bg-[#09090b] text-white relative overflow-hidden">
        ...
        <div className="mx-auto max-w-[1280px] px-3 sm:px-4 md:px-6 py-8 space-y-8">
          <section className="grid lg:grid-cols-3 gap-4">
            {[
              ...
              <div key={item.title} className="rounded-[16px] border border-white/[0.06] bg-white/[0.03] backdrop-blur p-5">
          ...
          <section className="grid lg:grid-cols-[1.2fr_0.8fr] gap-8">
            <div className="space-y-6">
              <div className="rounded-[16px] border border-white/[0.06] bg-white/[0.03] backdrop-blur p-6">
                ...
                <div className="mt-4 grid sm:grid-cols-2 gap-3 text-sm">
                  <div className="rounded-xl bg-white/[0.04] backdrop-blur p-3"><div className="label-ceremonial">Penyelenggara</div><div className="font-bold">PASKIBRA SMKN 1 KERTOSONO</div><div className="text-xs text-muted-foreground">Satria Cengkara</div></div>
                  <div className="rounded-xl bg-white/[0.04] backdrop-blur p-3"><div className="label-ceremonial">Lokasi</div><div className="font-bold">SMKN 1 KERTOSONO</div><div className="text-xs text-muted-foreground">Kertosono, Nganjuk, Jawa Timur</div></div>
                </div>
              </div>
              ...
              <div className="rounded-[16px] border border-white/[0.06] bg-white/[0.03] backdrop-blur p-6">
                ...
                <div className="mt-3 grid sm:grid-cols-2 gap-3">
                  {judges && judges.length>0 ? judges.map((j:any)=> (
                    <div key={j.id || j.name} className="flex items-center gap-3 rounded-xl border border-white/[0.06] p-3">
                      <img src={j.photo_url || "/assets/brand/lkbb-logo.jpg"} alt={j.name} className="h-10 w-10 rounded-full object-cover border bg-white/[0.04] backdrop-blur" />
              ...
              <div className="rounded-[16px] border border-white/[0.06] bg-white/[0.03] backdrop-blur p-6">
                ...
                <div className="mt-3 grid sm:grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={t} className="rounded-xl border border-white/[0.06] p-4 text-center">
            ...
            <div className="space-y-4">
              <div className="rounded-[16px] border border-white/[0.06] overflow-hidden bg-white/[0.03] backdrop-blur">
        ...
        {sponsorsEnabled && sponsors.length>0 && (
          <section className="border-t border-white/[0.06] bg-white/[0.03] backdrop-blur mt-2">
            <div className="mx-auto max-w-[1280px] px-3 sm:px-4 md:px-6 py-8 sm:py-10">
              ...
              <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
                {sponsors.map((s:any)=> (
                  <div key={s.id} className="group rounded-[16px] border border-white/[0.06] bg-white/[0.04] backdrop-blur/20 hover:bg-white/[0.03] backdrop-blur hover:border-[#C9A86A]/30 hover:shadow-soft p-4 flex flex-col items-center justify-center text-center transition-all min-h-[110px]">
                    {s.logo_url ? (
                      <img src={s.logo_url} alt={s.name} className="h-12 w-full object-contain mb-2" />
                    ) : (
                      <div className="h-12 w-full grid place-items-center rounded-lg bg-white/[0.03] backdrop-blur border text-xs font-black mb-2">{s.name.slice(0,12)}</div>
                    )}

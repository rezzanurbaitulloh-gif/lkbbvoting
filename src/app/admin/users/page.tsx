      ...
      <div className="rounded-[16px] border border-border bg-card overflow-hidden">
        ...
        <div className="hidden md:block overflow-x-auto">
          ...
          {users.length===0 ? <div className="p-8 text-center text-sm text-muted-foreground">Belum ada pengguna.</div> :
            users.map((u:any)=> (
            <div key={u.id} className="grid grid-cols-[40px_1.4fr_1.2fr_120px_100px] gap-2 px-4 py-3 items-center border-b border-border/50 text-sm">
        ...
        <div className="md:hidden space-y-2 p-3">
          {users.length===0 ? <div className="p-6 text-center text-sm text-muted-foreground">Belum ada pengguna.</div> :
            users.map((u:any)=> (
            <div key={u.id} className="rounded-xl border border-border p-3 flex gap-3">

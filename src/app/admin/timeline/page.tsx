...
export default function Page(){
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between"><div><h1 className="text-[18px] font-black">Timeline</h1><p className="text-sm text-muted-foreground">{list.length} data dari DB</p></div><Button size="sm" className="rounded-full" onClick={openAdd}>Tambah Baru</Button></div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[520px] max-h-[85vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editing ? "Edit Timeline" : "Tambah Timeline"}</DialogTitle><DialogDescription>Isi semua field yang wajib.</DialogDescription></DialogHeader>
          <div className="grid gap-3">
            <div><label className="text-xs font-bold">Judul *</label><Input value={form.title} onChange={e=> setForm({...form, title:e.target.value})} placeholder="Nama tahapan" /></div>
            <div><label className="text-xs font-bold">Tanggal</label><Input value={form.date} onChange={e=> setForm({...form, date:e.target.value})} placeholder="Oktober 2026" /></div>
            <div><label className="text-xs font-bold">Deskripsi</label><textarea value={form.description} onChange={e=> setForm({...form, description:e.target.value})} placeholder="Deskripsi tahapan" className="w-full min-h-[80px] rounded-xl border border-input bg-background px-3 py-2 text-sm" /></div>
            <div><label className="text-xs font-bold">Status</label><Select value={String(form.status)} onValueChange={v=> setForm({...form, status:v})} options={[{value:"completed",label:"Selesai"},{value:"current",label:"Berlangsung"},{value:"upcoming",label:"Akan Datang"}]} /></div>

    ...
    <div className="rounded-[16px] border border-white/10 bg-white/5 backdrop-blur p-4">
      ...
      <div className="mt-3 space-y-1">
        {list.length===0 ? <div className="p-6 text-center text-sm text-muted-foreground">Belum ada ranking.</div> :
          list.map((p,i)=> (
          <div key={p.id} className="flex items-center gap-3 rounded-xl border border-white/10 p-2.5">
            <div className={`h-7 w-7 rounded-full grid place-items-center text-xs font-black shrink-0 ${i===0 ? "bg-[#C9A86A] text-[#0B0C0F]" : i===1 ? "bg-zinc-300 text-black" : i===2 ? "bg-amber-700 text-white" : "bg-white/5 backdrop-blur"}`}>{i+1}</div>
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-5">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="rounded-[16px] border border-[#C9A86A]/20 bg-white/5 backdrop-blur p-4">
          <div className="flex items-center justify-between">

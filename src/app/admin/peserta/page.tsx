    ...
    <div className="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-5">
      ...
      <div className="rounded-[16px] border border-white/10 bg-white/5 backdrop-blur overflow-hidden">
        ...
        <div className="hidden md:block overflow-x-auto">
          <div className="min-w-[560px] grid grid-cols-[40px_1.4fr_1.2fr_110px_90px] gap-2 px-4 py-3 text-[11px] font-bold tracking-widest text-muted-foreground border-b border-white/10 bg-white/5 backdrop-blur/30">
          ...
          {list.length===0 ? <div className="p-8 text-center text-sm text-muted-foreground">Belum ada peserta.</div> :
            list.map((p:any)=> (
            <div key={p.id} className="min-w-[560px] grid grid-cols-[40px_1.4fr_1.2fr_110px_90px] gap-2 px-4 py-3 items-center border-b border-white/10/50 text-sm">
        ...
        <div className="md:hidden space-y-2 p-3">
          {list.length===0 ? <div className="p-6 text-center text-sm text-muted-foreground">Belum ada peserta.</div> :
            list.map((p:any)=> (
            <div key={p.id} className="rounded-xl border border-white/10 p-3 flex gap-3">

  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[18px] font-black">Sponsor</h1>
          <p className="text-sm text-muted-foreground">Kelola sponsor dan mitra</p>
        </div>
        ...
        <div className="grid gap-2">
          {[1,2,3].map(i=> (
            <div key={i} className="flex items-center justify-between rounded-xl border border-border p-3">
              <div>
                <div className="text-sm font-bold">Contoh Sponsor #{i}</div>
      ...
      <div className="rounded-xl border border-dashed border-border bg-muted/20 p-4 text-center text-xs text-muted-foreground">
        CRUD untuk Sponsor — Create, Edit, Publish, Unpublish, Delete, Reorder. Data terhubung ke backend via service abstraction.
      </div>
    </div>
  )
}

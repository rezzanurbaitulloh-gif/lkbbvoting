...
export default function Verifikasi(){
  ...
  useEffect(()=>{ const s=createBrowserSupabase(); s.from("peletons").select("*").order("created_at",{ascending:false}).then(({data})=> setAll(data||[])) },[])
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      ...
      <div className="grid gap-3">
        ...
        {(pending.length ? pending : all.slice(-2)).map(p=> (
          <div key={p.id} className="rounded-[16px] border border-border bg-card p-4 flex gap-4">
            <img src={p.image_url || p.image} alt="" className="h-16 w-16 rounded-xl object-cover border" />
            <div className="flex-1 min-w-0">
              <div className="text-sm font-black">{p.name} • #{p.number}</div>
              <div className="text-xs text-muted-foreground">{p.school} • {p.city} • {new Date(p.created_at || p.createdAt).toLocaleDateString("id-ID")}</div>

...
export function LeaderboardPreview(){
  ...
  useEffect(()=>{
    ...
    supabase.from("team_ranking").select("*").eq("category","SMP").order("total_ballots",{ascending:false}).limit(5).then(({data})=> setSmp(data||[]))
    supabase.from("team_ranking").select("*").eq("category","SMA").order("total_ballots",{ascending:false}).limit(5).then(({data})=> setSma(data||[]))
  },[])
  const Row = ({p,i}:{p:any,i:number})=> (
    <div className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.03] backdrop-blur px-3 py-2.5">
      <div className={`h-8 w-8 rounded-full grid place-items-center text-xs font-black ${i<3 ? "bg-gold text-gold-foreground" : "bg-white/[0.04] backdrop-blur text-muted-foreground"}`}>{i+1}</div>
      <img src={p.image_url || p.image} alt="" className="h-9 w-9 rounded-full object-cover border border-white/[0.06]" />

    ...
    <div className="group relative flex flex-col overflow-hidden rounded-[16px] border border-border bg-card shadow-subtle hover:shadow-soft transition-all duration-300">
      <Link href={profileUrl} className="block">
        <div className="relative aspect-[4/3] overflow-hidden bg-muted">
          ...
          <button onClick={(e)=>{ e.preventDefault(); e.stopPropagation(); handleFav(); }} aria-label="favorite" className={cn("absolute right-3 top-3 h-8 w-8 grid place-items-center rounded-full backdrop-blur border transition-colors", fav ? "bg-[#A51D2D] border-[#A51D2D] text-white" : "bg-black/40 border-white/15 text-white hover:bg-black/60")}>

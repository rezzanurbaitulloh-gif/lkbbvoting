    ...
    <div className="group relative flex flex-col overflow-hidden rounded-[16px] border border-white/5 bg-card premium-card max-w-full">
      ...
      <Link href={profileUrl} className="block">
        <div className="relative aspect-[4/3] overflow-hidden bg-muted">
          ...
          <div className="absolute left-2 sm:left-3 top-2 sm:top-3 rounded-full bg-black/70 backdrop-blur px-2 sm:px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-[11px] font-black tracking-widest text-white border border-white/10">
            #{number}
          </div>
          <button onClick={(e)=>{ e.preventDefault(); e.stopPropagation(); handleFav(); }} aria-label="favorite" className={cn("absolute right-2 sm:right-3 top-2 sm:top-3 h-7 w-7 sm:h-8 sm:w-8 grid place-items-center rounded-full backdrop-blur border transition-colors", fav ? "bg-[#A51D2D] border-[#A51D2D] text-white" : "bg-black/40 border-white/15 text-white hover:bg-black/60")}>
            <Heart className={cn("h-3 w-3 sm:h-3.5 sm:w-3.5", fav && "fill-white")} />
          </button>
          <div className="absolute bottom-2 sm:bottom-3 right-2 sm:right-3 rounded-full bg-white/90 backdrop-blur px-2 sm:px-2.5 py-0.5 sm:py-1 text-[9px] sm:text-[10px] font-bold tracking-widest text-black border border-white/20 max-w-[45%] truncate">
            {category}
          </div>
        </div>
        <div className="flex flex-col p-3 sm:p-4 pb-2">
          <div className="flex gap-2.5 sm:gap-3">
            <div className="flex-1 min-w-0">
              <div className="text-[10px] sm:text-[11px] font-bold tracking-[0.12em] sm:tracking-[0.14em] text-gold">#{number}</div>
            ...
            <img src={logo} alt="logo" className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl object-cover border border-border bg-muted shrink-0" />
          </div>
        </div>
      </Link>
      <div className="flex flex-col gap-2 p-3 sm:p-4 pt-2">
        <Link href={supportUrl} className="block">
          <Button size="sm" className="w-full rounded-full h-8 sm:h-9 text-[13px] sm:text-sm font-black tracking-wide">DUKUNG</Button>
        </Link>
